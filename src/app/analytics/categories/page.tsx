...
export default function CategoryIntelligencePage() {
  ...
  const { data, loading } = useApi<any>(`/api/admin/analytics/categories?period=${period}`);
  ...
  // Decorative bubble palette — kept as literal hex because these are chart-series
  // colours, not theme surfaces. They read fine on both light and dark backgrounds.
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-fg mb-1">Category Intelligence</h1>
          <p className="text-fg-muted text-sm">Performance matrix, trends, and pricing analysis across categories</p>
        </div>
      ...
      <div className="bg-panel border border-muted rounded-xl p-6">
        <h2 className="text-lg font-semibold text-fg mb-2">Category Performance Matrix</h2>
        <p className="text-fg-muted text-xs mb-4">X: Demand Volume | Y: Conversion Rate | Bubble Size: Revenue</p>
        {categories.length === 0 ? (
          <div className="text-sm text-fg-muted p-4 text-center">No category data available</div>
        ) : (
        <svg viewBox={`0 0 ${scW} ${scH}`} className="w-full" preserveAspectRatio="xMidYMid meet">
          {/* Grid */}
          {[0, 0.25, 0.5, 0.75, 1].map(f => {
            const y = scPad.top + scInnerH * (1 - f);
            return <line key={`h${f}`} x1={scPad.left} y1={y} x2={scW - scPad.right} y2={y} stroke="var(--color-border-muted)" strokeWidth="0.5" />;
          })}
          {/* Quadrant lines */}
          {(() => {
            ...
            return (
              <>
                <line x1={mx} y1={scPad.top} x2={mx} y2={scPad.top + scInnerH} stroke="var(--color-border-muted)" strokeWidth="1" strokeDasharray="4,4" />
                <line x1={scPad.left} y1={my} x2={scW - scPad.right} y2={my} stroke="var(--color-border-muted)" strokeWidth="1" strokeDasharray="4,4" />
                {/* Quadrant labels */}
                <text x={scPad.left + 5} y={scPad.top + 15} fill="var(--color-fg-muted)" fontSize="10">Question Marks</text>
                <text x={scW - scPad.right - 5} y={scPad.top + 15} textAnchor="end" fill="var(--color-fg-muted)" fontSize="10">Stars</text>
                <text x={scPad.left + 5} y={scPad.top + scInnerH - 5} fill="var(--color-fg-muted)" fontSize="10">Dogs</text>
                <text x={scW - scPad.right - 5} y={scPad.top + scInnerH - 5} textAnchor="end" fill="var(--color-fg-muted)" fontSize="10">Cash Cows</text>
              </>
            );
          })()}
          {/* Axis labels */}
          <text x={scW / 2} y={scH - 5} textAnchor="middle" fill="var(--color-fg-muted)" fontSize="11">Demand Volume</text>
          <text x={15} y={scH / 2} textAnchor="middle" fill="var(--color-fg-muted)" fontSize="11" transform={`rotate(-90, 15, ${scH / 2})`}>Conversion Rate</text>
          {/* Dots */}
          {categories.map((cat: any, i: number) => {
            ...
            return (
              <g key={cat.categoryId}>
                ...
                <text x={x} y={y - r - 3} textAnchor="middle" fill="var(--color-fg)" fontSize="8">{cat.categoryName}</text>
              </g>
            );
          })}
        </svg>
      ...
      <div className="bg-panel border border-muted rounded-xl p-6">
        <h2 className="text-lg font-semibold text-fg mb-4">Category Trends ({period} Days)</h2>
        <div className="space-y-3">
          <div className="grid grid-cols-[1fr_120px_80px_80px] gap-2 text-xs text-fg-muted font-medium pb-2 border-b border-muted">
            <span>Category</span>
            <span>Trend</span>
          ...
          {sparklines.length === 0 && (
            <div className="py-4 text-sm text-fg-muted text-center">No category trend data</div>
          )}
          {sparklines.map((cat: any, idx: number) => {
            ...
            return (
              <div key={cat.categoryId} className="grid grid-cols-[1fr_120px_80px_80px] gap-2 items-center py-1">
                <span className="text-sm text-fg">{cat.categoryName}</span>
                ...
                <span className="text-sm text-fg text-right">{cat.demandCount}</span>
                <span className={`text-sm text-right font-medium ${(cat.change || 0) >= 0 ? 'text-accent' : 'text-danger'}`}>
      ...
      {/* Price Intelligence */}
      <div className="bg-panel border border-muted rounded-xl p-6">
        <h2 className="text-lg font-semibold text-fg mb-4">Price Intelligence</h2>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-muted">
                <th className="text-left py-2 text-fg-muted font-medium">Category</th>
                <th className="text-right py-2 text-fg-muted font-medium">Avg Buyer Budget</th>
                <th className="text-right py-2 text-fg-muted font-medium">Avg Seller Price</th>
                <th className="text-right py-2 text-fg-muted font-medium">Gap %</th>
              </tr>
            </thead>
            <tbody>
              {priceData.length === 0 && (
                <tr><td colSpan={4} className="py-8 text-center text-sm text-fg-muted">No price data available</td></tr>
              )}
              {priceData.map((row: any) => {
                const gapColor = Math.abs(row.gap) > 30 ? 'var(--color-danger)' : Math.abs(row.gap) > 15 ? 'var(--color-warning)' : 'var(--color-accent)';
                return (
                  <tr key={row.category} className="border-b border-muted/50">
                    <td className="py-2 text-fg">{row.category}</td>
                    <td className="py-2 text-right text-fg">${row.buyerBudget}</td>
                    <td className="py-2 text-right text-fg">${row.sellerPrice}</td>
